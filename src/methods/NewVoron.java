...
public class NewVoron
{
	...
	public static void startMethod(int N, double error, double[][] initialX, int numberOfIteration)
	{
		...
		do
		{
			...
			for (int i=0; i<3; i++)
			{
				if(Math.abs(b[i][0]-middle)>sigma)
				{
					sigma = Math.abs(b[i][0]-middle);
					maxDefectionB = i;
				}
				
			}
			*/
			//b[maxDefectionB][0] = b_old[maxDefectionB][0];
			b[nFixB][0]=fixB;
			fixB = b[0][0];
			nFixB = 0;
			for (int i = 0; i < 3; i++)
		...
		System.out.println("Coefficients b of NewVoronzov method");
		for (int i = 0; i < 3; i++)
		{
